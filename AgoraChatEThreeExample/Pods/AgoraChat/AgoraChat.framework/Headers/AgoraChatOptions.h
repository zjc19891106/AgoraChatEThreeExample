...
typedef enum
{
    AgoraChatLogLevelDebug = 0, /** \~chinese 输出所有日志。 \~english Output all logs. */
    AgoraChatLogLevelWarning,   /** \~chinese 输出警告及错误。 \~english Output warnings and errors. */
    AgoraChatLogLevelError      /** \~chinese 只输出错误。 \~english Output errors only. */
} AgoraChatLogLevel;
...
/**
 *  \~chinese
 *  默认值为 `AgoraChatLogLevelDebug`，表示所有等级的日志。
 *
 *  \~english
 ...
 *  已废弃。
 *
 *  只能在[AgoraChatClient initializeSDKWithOptions:]时设置，不能在程序运行过程中动态修改。
 *
 *  \~english
