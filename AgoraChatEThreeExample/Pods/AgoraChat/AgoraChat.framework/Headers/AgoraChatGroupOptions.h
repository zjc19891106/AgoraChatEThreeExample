 ...
 */
typedef enum{
    AgoraChatGroupStylePrivateOnlyOwnerInvite  = 0,    /**! \~chinese 私有群组类型，只允许群主邀请用户加入。 \~english A private group, where only the group owner can invite users to join. */
    AgoraChatGroupStylePrivateMemberCanInvite,         /**! \~chinese 私有群组类型，群主和群成员均可邀请用户加入。 \~english A private group, where all the group members including the group owner, group manager, and other group members can invite users to join.  */
    AgoraChatGroupStylePublicJoinNeedApproval,         /**! \~chinese 公开群组类型，群主可以邀请用户加入; 非群成员用户发送入群申请，经群主同意后才能入组。 \~english A public group, where the group owner can invite users to user. Users can send a join request and join the group if the owner approves it. */
    AgoraChatGroupStylePublicOpenJoin,                 /**! \~chinese 公开群组类型，用户可以自由加入。 \~english A public group, where users can join freely. */
}AgoraChatGroupStyle;
