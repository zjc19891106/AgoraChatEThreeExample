 ...
 */
typedef enum{
    AgoraChatGroupLeaveReasonBeRemoved = 0,    /** \~chinese 成员被群主移除。 \~english The member is removed by the group owner. */
    AgoraChatGroupLeaveReasonUserLeave,        /** \~chinese 成员主动离开群组。 \~english The member leaves the group.*/
    AgoraChatGroupLeaveReasonDestroyed,        /** \~chinese 群组被销毁。 \~english The group is destroyed.*/
 ...
 *  自动加入群组回调。
 * 
 *  如果你在 AgoraChatOptions 中将 isAutoAcceptGroupInvitation 设为 YES，则在收到其他用户的群组邀请后，SDK 会自动进群，并触发该回调。
 * 
 * 
 *  @param aGroup    群组实例。
 *  @param aInviter  邀请者。
 ...
 *  群主收到用户入群申请回调。
 * 
 *  如果你将群组类型设置为 AgoraChatGroupStylePublicJoinNeedApproval，则用户申请入群时，群主会收到该回调。
 *
 *  @param aGroup     群组实例。
 ...
 *  群主拒绝入群申请回调。
 * 
 *  如果你将群组类型设为 AgoraChatGroupStylePublicJoinNeedApproval，则群主拒绝用户的入群申请后，该用户会收到该回调。
 * 
 *  @param aGroupId    群组 ID。
 *  @param aReason     拒绝理由。
 *
 ...
 * 加入群组申请已同意回调。
 * 
 *  如果你将群组类型设为 AgoraChatGroupStylePublicJoinNeedApproval，则群主同意用户的入群申请后，该用户会收到该回调。 
 * 
 ...
 *  群组列表发生变化回调。
 *
 *  @param aGroupList  群组列表，详见 <AgoraChatGroup>。
 *
 *  \~english
...
/**
 *  \~chinese
 *  SDK 自动同意了用户 A 的加 B 入群邀请后，用户 B 接收到该回调，需要设置 AgoraChatOptions 的 isAutoAcceptGroupInvitation 为 YES。
 * 
 ...
 * 
 *  Sets AgoraChatOptions's isAutoAcceptGroupInvitation property to YES for this delegate method.
 * 
...
/**
 *  \~chinese
 *  群主收到用户的入群申请，群的类型是 AgoraChatGroupStylePublicJoinNeedApproval。
 * 
 ...
 *
 *  \~english
 *  The group owner receives user's application of joining group. The group's style is AgoraChatGroupStylePublicJoinNeedApproval.
 * 
...
/**
 *  \~chinese
 *  群主拒绝用户 A 的入群申请后，用户 A 会接收到该回调，群的类型是 AgoraChatGroupStylePublicJoinNeedApproval。
 * 
...
/**
 *  \~chinese
 *  群主同意用户 A 的入群申请后，用户 A 会接收到该回调，群的类型是 AgoraChatGroupStylePublicJoinNeedApproval。
 * 
 ...
 *
 *  \~english
 *  User A will receive this callback after group's owner accepted it's application. The group's style is AgoraChatGroupStylePublicJoinNeedApproval.
 * 
 ...
 *
 *  @param aGroupList  The group list. <AgoraChatGroup>
 */
